...
class BookingSearch:
    ...
    def controller(cls, driver:webdriver.Chrome, _now:dt.now=dt.now(), date_end=list(), occupancy=2, start=4, p:ProcessActive=None):
        ...
        try:
            ...
            print("Error in button cookies, element not fount")
        except ElementClickInterceptedException as e:
            print("Error in button cookies, element not clicked")
        ...
        try:
            while True:
                try:
                    ...
                    b2.click()
                    sleep(1)
                    divs2 = driver.find_elements_by_xpath("//div[@class='bfb38641b0']")
                    print(divs2[0].text)
                    for i in range(occupancy - int(divs2[0].text)):
                        buttons2 = divs2[0].find_elements_by_tag_name("button")
                        buttons2[1].click()
                except Exception as err01:
                    print("Error in button occupancy: "+str(err01))
                try:
                    error = None
                    _date = driver.find_element_by_xpath("//div[@data-testid='searchbox-dates-container']")
                    _date.click()
                    divs = driver.find_elements_by_xpath("//div[@class='d358556c65']")
                ...
                if error is None:
                    ...
                    try:
                        ...
                        print("Error in button category order")
                    
                    sleep(2)
                    #pg_state = False
                    #try:
                    #    button_elem, _pg_end = cls.pagination(driver.page_source, 2)
                    #    pg_state = True
                    ...
                    # Get data pagination
                    #if pg_state:
                    #for i in range(2, _pg_end+1):
                    # Items booking search
                    items = driver.find_elements_by_xpath("//div[@data-testid='property-card']")
                    #print(len(items))
                    
                    for pt in p.position:
                        print(pt)
                    # para 4 start y 2 persons.
                    if start == 4 and occupancy == 2:
                        # obtiene la posicion 1,5,10,15,20,25
                        for k in range(0, len(items) + 1, 5):
                            #print(k if k == 0 else k-1)
                            position = k if k == 0 else k-1
                            aux_d = cls.get_data_to_text(items[position], _date_elem, _now, occupancy, position)
                            if aux_d["title"] not in list(item_list.keys()):
                                item_list[aux_d["title"]] = []
                            item_list[aux_d["title"]].append(aux_d)
                    elif start == 3 and occupancy == 2:
                        # obtiene la posicion 3
                        position = 2
                        aux_d = cls.get_data_to_text(items[position], _date_elem, _now, occupancy, position)
                        if aux_d["title"] not in list(item_list.keys()):
                            item_list[aux_d["title"]] = []
                    ...
                    elif start == 4 and occupancy == 3:
                        # obtiene la posicion 1, 5, 10
                        for k in range(0, len(items) + 1, 5):
                            #print(k if k == 0 else k-1)
                            position = k if k == 0 else k-1
                            aux_d = cls.get_data_to_text(items[position], _date_elem, _now, occupancy, position)
                    ...
                    #if button_elem:
                    #    buttons = driver.find_elements_by_xpath("//button[@class='"+button_elem[0]+" "+button_elem[1]+"']")
                    #    for bt in buttons:
                    #        if int(str(bt.get_attribute("innerHTML")).strip()) == i:
                    #            bt.click()
                    #            break
                    #try:
                    #    button_elem, _ = cls.pagination(driver.page_source, i+1)
                    #except:
                    #    pass
                    #print("Items:",len(item_list), "| Pg:",i)
                    sleep(1)

                    #data = cls.filter_data(item_list)
                    #cls.write_file(data, str(_date_elem.date()))
                    # End data paginations
                        
                    if _date_elem.date() >= _date_end.date():
                        break
                    cont += 1
        except Exception as e2:
            print("Error 218: "+str(e2))
    ...
    @classmethod
    def get_data_to_text(cls, item, _date_elem, _now, occupancy, position):
        try:
            item_dict = {
                "start": 0,
                "title": "",
            ...
            try:
                ...
                print("[-] Error in Get start")
            try:
                item_dict["title"], item_dict["link"] = cls.search_title(html)
            except Exception as e3:
                print("[-] Error in Get title and link")
            try:
            ...
            try:
                ...
                print("[-] Error in Get description")
            try:
                item_dict["img"] = cls.search_img(html)
            except Exception as e5:
                print("[-] Error in Get img")
            try:
            ...
            bg = Booking.objects.filter(title=item_dict["title"]).first()
            if not bg:
