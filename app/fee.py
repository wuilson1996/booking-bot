...
class FeeTask:
    ...
    @classmethod
    def controller(cls, driver:webdriver.Chrome, price, tipo, _date:str, username, password):
        try:
            driver.get(cls._url)
            ...
            sleep(5)
            while True:
                if "Optimizando..." not in driver.page_source:
                    break
            while True:
                status = False
                for b in driver.find_elements_by_xpath("//button[@data-state='closed']"):
                    if b.get_attribute("data-testid") != "" and b.get_attribute("data-testid") != None:
                        #print(f"[+] Fecha: {str(b.get_attribute('data-testid'))}...")
                        if _date == str(b.get_attribute("data-testid")):
                            print(f"[+] Fecha encontrada: {str(b.get_attribute('data-testid'))}...")
                            status = True
                            b.click()
                            sleep(3)
