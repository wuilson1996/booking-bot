            ...
            while True:
                ...
                if _error is None:
                    ...
                    try:
                        ...
                        for s in elements:
                            logging.info(f"[+] {dt.now()} - {str(start)} stars - Start: {s.get('aria-label')}")
                            if str(start)+" estrellas" in str(s.get('aria-label')):# or str(start)+" stars" 
                        ...
                        try:
                            ...
                            for s in elements:
                                logging.info(f"[+] {dt.now()} - {start} - {str(start)+' stars' in s.get('aria-label')} - Start: {s.get('aria-label')}")
                                if str(start)+" estrellas" in str(s.get('aria-label')):#or str(start)+" stars"
                                    driver.find_element_by_xpath("//input[@id='"+str(s.get("id"))+"']").click()
                                    logging.info(f"[+] {dt.now()} Click button start success")
                                    break
