...
class BookingSearch:
    ...
    def _driver_chrome(cls, url) -> None:
        ...
        options.add_argument("--disable-cache")  # Desactivar caché
    ...
    @classmethod
    def controller(cls, driver, _now:dt.now=dt.now(), process:ProcessActive=None, search_name="", alert=True):
        try:
            _current_url = cls._url
            if not alert:
                _current_url = cls._url_performance
            else:
            ...
            try:
                ...
                logging.info(f"[-] {dt.now()} Error in button cookies, exception")
            # Search
            # add validation Este alojamiento no tiene disponibilidad
            ...
            logging.info(f"[-] {dt.now()} - {_date_elem.date()} - {_now.date()} - S:{process.start} - O:{process.occupancy} - {driver.current_url}")
            ...
            if process.type_proces == 1:
                try:
                    ...
                    try:
                        ...
                        for s in elements:
                            #logging.info(f"[+] {dt.now()} - Log2 - {str(process.start)} stars - Input: {s.get('aria-label')}")
                            if str(process.start)+" stars" == str(s.get('aria-label')).split(":")[0].strip() or str(process.start)+" stars" == str(s.get('aria-label')).split("/")[0].strip() or str(process.start)+" estrellas" == str(s.get('aria-label')).split(":")[0].strip():#or str(process.start)+" stars"
